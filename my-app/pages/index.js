...
export default function Home() {
  ...
  // User State
  const [nftBalance, setNftBalance] = useState(0); // User's balance of CryptoDevs NFTs
  ...
  const fetchProposalById = async (id) => {
    try {
      ...
      const parsedProposal = {
        ...
        votesYes: proposal.votesY.toString(),
        votesNo: proposal.votesN.toString(),
  ...
  // Runs everytime the value of `walletConnected` changes. Prompts user to connect wallet if not connected, and then
  // calls helper functions to fetch the DAO Treasury Balance, User NFT Balance, and Number of Proposals in the DAO.
  useEffect(() => {
    if (!walletConnected) {
  ...
  // Runs everytime the value of `selectedTab` changes. Used to re-fetch all proposals in the DAO when user switches
        ...
        <div>
          {proposals.map((p, index) => (
            <div key={index} className={styles.proposalCard}>
              ...
              <p>Votes For: {p.votesYes}</p>
              <p>Votes Against: {p.votesNo}</p>
